...
def aplicar_regra_noite(resultados):
    """
    Noite começa no dia anterior:
    - Dia anterior Descanso → Noite
    - Dia anterior Manhã    → M+N
    - Dia anterior Tarde    → T+N
    - Dia da Noite          → Descanso
    - Se não há dia anterior (dia 1) → Descanso
    """
    turnos = {dia: turno for dia, turno in resultados}
    novo = dict(turnos)
...
def extrair_turnos(img,mes,ano):
    ...
    resultados = [(d, classificar_celula(celulas[r][c])) for d,r,c in posicoes]
    return aplicar_regra_noite(resultados)
